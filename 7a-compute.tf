...
resource "google_compute_instance" "sample-vm" {    # Change resource name of google_compute_instance to a name of your choosing i.e. "test-vm"
  ...
  tags = ["test"]                    # Change zone name "us-central1-a" to zone in the region availability zone you wish to house your VMs in i.e., "us-east4-a" 
...
# If you wish to check what zones and regions are available run the command gcloud compute zones list 
  ...
  network_interface {
    subnetwork = google_compute_subnetwork.jourdan-linux-private-subnet.name   # Change the hqinternal part of google_compute_subnetwork.hqinternal.name to reference the name(s) of your google compute subnetwork(s) that you created in file number 3    
    access_config {
      // Ephemeral public IP
    }
  }
  ...
  metadata_startup_script = file("./startup.sh")             # If you do not have a startup.sh file, this function will not work
                            # Change the main part of google_compute_firewall.web_traffic.name to reference the name(s) of your google compute firewall(s) that you created in file number 6
}